...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let leaf = self.nodes.get_mut(&ppn).unwrap();
        leaf.entries[vpn] = ((pa >> 12) << PPN_SHIFT) | flags | PTE_V;
    }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for level in (0..3).rev() {
            ...
            if is_leaf {
                ...
                let pa_top = (leaf_ppn << 12) & !offset_mask;
                return TranslateResult::Ok(pa_top | (va & offset_mask));
            }
            if level == 0 {
                // level 0 必须是叶子
                return TranslateResult::PageFault;
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let pte = self.nodes.get(&self.root_ppn).unwrap().entries[vpn2];
        let mid_ppn = if pte & PTE_V == 0 {
            let new_ppn = self.alloc_node();
        ...
        let mid = self.nodes.get_mut(&mid_ppn).unwrap();
        mid.entries[vpn1] = ((pa >> 12) << PPN_SHIFT) | flags | PTE_V;
    }
}
