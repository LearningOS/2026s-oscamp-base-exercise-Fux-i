...
pub fn double_in_thread(numbers: Vec<i32>) -> Vec<i32> {
    ...
    thread::spawn(move || numbers.iter().map(|num| num * 2).collect())
        .join()
        .unwrap()
}
...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    ...
    let suma = thread::spawn(move || a.iter().sum::<i32>()).join().unwrap();
    let sumb = thread::spawn(move || b.iter().sum::<i32>()).join().unwrap();
    (suma, sumb)
}
...
pub fn named_sleeper(value: i32, ms: u64) -> i32 {
    ...
    let s = thread::Builder::new().name("sleeper".to_string());
    s.spawn(move || {
...
pub fn scoped_slice_sum(a: &[i32], b: &[i32]) -> (i32, i32) {
    ...
    thread::scope(|s| {
        let suma = s.spawn(|| a.iter().sum::<i32>());
        let sumb = s.spawn(|| b.iter().sum::<i32>());
...
pub fn handle_panic(value: i32, should_panic: bool) -> Result<i32, ()> {
    ...
    let handle = thread::spawn(move || if should_panic { panic!("oops") } else { value });
    match handle.join() {
        Ok(_) => Ok(value),
        Err(_) => Err(()),
    }
