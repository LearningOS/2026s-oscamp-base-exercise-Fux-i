...
impl TaskContext {
    ...
    pub fn init(&mut self, stack_top: usize, entry: usize) {
        use core::arch::asm;
        unsafe {
            asm!("mv {}, {}", out(reg) self.ra, in(reg) entry);
            asm!("mv {}, {}", out(reg) self.sp, in(reg) stack_top);
        }
    }
}
...
pub unsafe extern "C" fn switch_context(old: &mut TaskContext, new: &TaskContext) {
    core::arch::naked_asm!(
        // Save callee-saved registers into *old (a0)
        "sd sp,   0(a0)",
        "sd ra,   8(a0)",
        "sd s0,  16(a0)",
        "sd s1,  24(a0)",
        ...
        "ld s10, 96(a1)",
        "ld s11, 104(a1)",
        // Do not leak context pointers into the new task
        "mv a0, zero",
        "mv a1, zero",
        // Jump to new.ra
        "jalr zero, 0(ra)",
...
pub fn alloc_stack() -> (Vec<u8>, usize) {
    ...
    // Stack must be 16-byte aligned per RISC-V ABI.
