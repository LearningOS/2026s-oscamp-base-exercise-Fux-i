...
impl Scheduler {
    ...
    fn schedule_next(&mut self) {
        ...
        // Round-robin search for the next Ready thread.
        let mut next = None;
        for i in 1..=n {
            let idx = (cur + i) % n;
        ...
        // If the next thread has a pending entry (first time it runs),
        // hand it off to the wrapper via the global.
        ...
        self.current = next;
        let old_ctx: *mut TaskContext = self.threads[cur].ctx.as_mut_ptr();
